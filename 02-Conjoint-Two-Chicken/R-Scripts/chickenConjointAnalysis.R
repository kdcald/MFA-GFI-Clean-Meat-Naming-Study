...
kcPath <- "/Users/Krystal/Google Drive/surveys/gfi/cultured_survey_chicken/analysis"
# gbPath <- ""
...
load("data/chickenDataForAnalysis.csv.Rdata")
#df <- dfConj
class(df)

# loads functions.
#source("r_code/vcovCluster.r")
source("r_code/ggplotAMCEs-2.R")
#source("r_code/conjointReshape_cultured.R")
...
df$productType <- (factor(df$productType, levels=c("Ground chicken", "Chicken patty", "Chicken nugget", "Chicken finger")))
df$price <- (factor(df$price, levels=c("$1.50/pound", "$2.50/pound","$3.75/pound", "$5.00/pound")))
...
# Binary MODELS WITH A DIFFERENT BASELINE
...
# PRODUCT CHOICE MODELS WITH A DIFFERENT BASELINE
